...
class LSTMClassifier(Classifier):
    ...
    def __load_model(self, model_path):
        ...
        if not os.path.exists(model_path) or not os.listdir(model_path):
            ...
            print('make download-checkpoint classifier={0} model={1}'.format(self.name, self.get_model_path()))
        try:
    ...
    def __get_text_features(self, question):
        ...
        univariate_features = textfeaturesgen.univariate_features(question)
        return univariate_features


    def __get_prediction(self, w2v_vector, topic_vector, text_features):
        model_path = self.get_model_path()
