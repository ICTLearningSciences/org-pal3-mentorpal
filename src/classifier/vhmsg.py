...
class VHMSG(object):
    ...
    def __init__(self, server = getServerFromEnvironment(), port = getPortFromEnvironment(), scope = getScopeFromEnvironment()):
        self.m_port   = port
        self.m_scope  = scope
    ...
    def sendMessage(self, firstWord, body):
        ...
        headers = {"ELVISH_SCOPE": self.m_scope, "MESSAGE_PREFIX" : firstWord, "type":"Text"}
        
        self.m_connection.send(destination=TOPIC_LABEL + self.m_scope, body=firstWord + " " + body, headers=headers, content_type="text/plain")
        return True
...
class VHMSGComponent(object):
    ...
    def __init__(self, componentName = "defaultComponentName", server = getServerFromEnvironment(), port = getPortFromEnvironment(), scope = getScopeFromEnvironment()):
