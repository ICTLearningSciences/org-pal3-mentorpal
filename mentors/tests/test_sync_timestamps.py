...
@pytest.mark.parametrize(
    ...
    [(MENTOR_DATA_ROOT, "mentor1-clean-slate")],
)
def test_it_generates_utterances_from_timestamps(
    mentor_data_root: str, mentor_id: str
):
    mp = MentorPath(mentor_id=mentor_id, root_path=mentor_data_root)
    expected_utterances = utterances_from_yaml(mp.get_mentor_path("expected-utterances.yaml"))
