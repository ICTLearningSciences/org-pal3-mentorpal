...
def format_and_save_pu_data(mentor, pu_data):
    ...
    pu_data.to_csv(os.path.join(os.getcwd(), MENTOR_DATA.format(mentor), PU_FILENAME))


def format_and_save_qpa_data(mentor, qpa_data):
    # Topic
    topic_map = get_master_map(os.path.join(os.getcwd(), SHARED_DATA, TOPIC_MAP))
    if topic_map.empty:
    ...
    # Paraphrase
    paraphrase_map = get_master_map(
        os.path.join(os.getcwd(), SHARED_DATA, PARAPHRASE_MAP)
    )
    ...
    qpa_data.to_csv(os.path.join(os.getcwd(), MENTOR_DATA.format(mentor), QPA_FILENAME))
