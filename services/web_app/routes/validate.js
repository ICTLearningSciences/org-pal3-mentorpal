  ...
  const filesID = ['utterance_data.csv', 'classifier_data.csv'];
	for (let k = 0; k<2;k++){
        ...
        for (let l = 0; l<filetypes.length; l++){
          const videoURL = `https://pal3.ict.usc.edu/resources/mentor/${mentorList[j]}/${rows[i]["ID"]}${filetypes[l]}`

          try {
            let res = await request(videoURL, { method: 'HEAD'})
            if(res.statusCode !== 200) {
              results.missing.push(videoURL)
              console.warn(`non-200 response to HEAD for ${videoURL}: ${res.statusCode}`)
            }
          }
          catch(err) {
            console.warn(`error response to HEAD for ${videoURL} ${err.message}`)
            results.missing.push(videoURL)
          }
  			}
  		}
