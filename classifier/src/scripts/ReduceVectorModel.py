    ...
    def compare(self, w1, w2, method=DOT_PROD_METHOD, dims=None):
        ...
        if method == DOT_PROD_METHOD:
            if w1vec is not None and w2vec is not None:
                if dims is None:
                    if len(w1vec) != len(w2vec):
                        raise IndexError(
                            "Length of word vector for %s (%i) != %s (%i)"
                            % (w1, len(w1vec), w2, len(w2vec))
                        )
                    dims = len(w1vec)
                try:
                    similarity = np.dot(
                        w1vec, w2vec
                    )  # sum([w1vec[i]*w2vec[i] for i in range(0,dims)])
                except IndexError:
                    raise IndexError(
                        "Length of word vector for %s (%i) != %s (%i)"
                        % (w1, len(w1vec), w2, len(w2vec))
            ...
            if w1vec is not None and w2vec is not None:
                ...
                try:
                    norm = (
                        float([w ** 2 for w in w1vec]) ** 0.5
                        + float([w ** 2 for w in w2vec]) ** 0.5
                    )
...
class Word2VecModel(BaseVectorModel):
    def __init__(self):
        self._vectorModel = KeyedVectors.load_word2vec_format(
...
def exampleTest():
    ...
    base = "abcd"
    allWords = [
        "".join(x) for i in range(2, len(base)) for x in itertools.permutations(base, i)
    ]
    print(allWords)
    ...
    dimMultiplier = 10
    modelData = {
        w: [w.count(x) / float(len(w)) for x in base] * dimMultiplier for w in allWords
    }
