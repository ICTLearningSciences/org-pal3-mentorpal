...
class TestSSL(unittest.TestCase):
    ...
    def test_ssl_connection(self):
        try:
            ...
            self.assertTrue(
                self.listener.messages == 1, "should have received 1 message"
            )
            self.assertTrue(
...
class TestSSLParams(unittest.TestCase):
    def setUp(self):
        ...
        self.transport = transport.Transport(host_and_ports=[self.host1, self.host2])
    ...
    def test_init_ssl_params(self):
        trans = transport.Transport(
            ...
            host_and_ports=[self.host1, self.host2],
        )
        for host_port in [self.host1, self.host2]:
            self.assertEqual(
                trans._Transport__ssl_params[host_port]["key_file"], self.ssl_key_file
            )
            self.assertEqual(
                trans._Transport__ssl_params[host_port]["cert_file"], self.ssl_cert_file
            )
            self.assertEqual(
                trans._Transport__ssl_params[host_port]["ca_certs"], self.ssl_ca_certs
            )
            self.assertEqual(
            ...
            self.assertEqual(
                trans._Transport__ssl_params[host_port]["ssl_version"], self.ssl_version
            )
