...
class NPCEditor(object):
    ...
    def load_test_data(self):
        self.test_data = json.load(
            open(
                os.path.join(
                    "mentors", self.mentor.id, "test_data", "lr_test_data.json"
                ),
                "r",
            )
        )
    ...
    def vhmsg_callback(self, firstWord, body):
        splitMsg = body.split(" ")
        if splitMsg[0] == "Clint":
            self.vhmsg.sendMessage(
                "vrAgentBML", "test {0} end complete".format(splitMsg[2])
            )
    ...
    def close_npceditor(self, id):
        self.vhmsg.sendMessage(
            "NPCEditor", '<script target="{0}">theDocument.close()</script>'.format(id)
        )
    ...
    def send_request(self, question):
        self.vhmsg.sendMessage(
            "vrSpeech", "start test{0} {1}".format(self.turn_id, self.mentor.id)
        )
        self.vhmsg.sendMessage(
            "vrSpeech",
            "finished-speaking test{0} {1}".format(self.turn_id, self.mentor.id),
        )
        self.vhmsg.sendMessage(
            "vrSpeech", "interp test{0} 0 1.0 normal {1}".format(self.turn_id, question)
        )
